...
from ticl.train_val import make_training_callback
from ticl.config_utils import compare_dicts, flatten_dict
...
if __name__=='__main__':
    ...
    orchestration = AttrDict({
        'experiment': 'MotherNet GHL',
        'description': 'Basic model.',
        'create_new_run': False, # Create as new MLFLow run, even if continuing
        'save_every': 10,
        'use_mlflow': True,
        'validate': True,
        'progress_bar': True,
        ...
        'wandb_overwrite': False,
        'extra_fast_test': False,
        'use_wandb': False,
        'continue_run': False,
        'load_strict': True, # Whether to load the architecture strictly when warm starting
        'restart_scheduler': False, # Whether to restart the scheduler when warm starting
    ...
    if orchestration.seed_everything:
    ...
    if orchestration.extra_fast_test:
        config['prior']['n_samples'] = 2 * 16
        config[attention_type]['nhead'] = 1
        config['dataloader']['min_eval_pos'] = 10
    ...
    dt = datetime.today()
    date_str = dt.strftime('%H_%M_%S__%d_%m_%Y')
    model_string = 'mn_basic_' + date_str
    ...
    if (not orchestration.use_mlflow) or mlflow_hostname is None:
        ...
        if orchestration.continue_run and not orchestration.create_new_run:
            ...
            run_args = {'run_name': model_string, 'description': orchestration.description}
    ...
    result = {'loss': total_loss, 'model': model, 'dataloader': dl,
