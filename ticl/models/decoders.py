...
class SummaryLayer(nn.Module):
    def __init__(self, emsize=512, embed_dim=2048, n_out=10, decoder_type='output_attention', nhead=4, cls_token_n=0):
        super().__init__()
        self.emsize = emsize
        self.n_out = n_out
        self.decoder_type = decoder_type
        self.nhead = nhead
        self.cls_token_n = cls_token_n
        ...
        if decoder_type == "output_attention":
            ...
            out_size = emsize
        elif decoder_type == "cls_token":
            out_size = emsize * cls_token_n
    ...
    def forward(self, x, y_src):
        if x.shape[0] != 0:
            if self.decoder_type == "output_attention":
                ...
                res = x[0]
            elif self.decoder_type == "cls_token":
                res = x[:self.cls_token_n].transpose(0, 1)
...
class MLPModelDecoder(nn.Module):
    def __init__(self, emsize=512, n_out=10, hidden_size=1024, decoder_type='output_attention', predicted_hidden_layer_size=None, embed_dim=2048,
                 ...
                 in_size=100, cls_token_n=0, low_rank_last=False):
        super().__init__()
        self.emsize = emsize
        # self.embed_dim = embed_dim
        self.n_out = n_out
        ...
        self.summary_layer = SummaryLayer(emsize=emsize, n_out=n_out, decoder_type=decoder_type, embed_dim=embed_dim, nhead=nhead, cls_token_n=cls_token_n)
        mlp_in_size = self.summary_layer.out_size
        if self.predicted_hidden_layers == 0:
            ...
            if not low_rank_last:
                ...
                self.num_output_layer_weights = n_out + n_out * self.weight_embedding_rank + self.in_size * \
                    self.weight_embedding_rank + self.predicted_hidden_layer_size
            if self.predicted_hidden_layers > 1:
            ...
            if low_rank_last:
                self.shared_weights.append(nn.Parameter(torch.randn(
                self.predicted_hidden_layer_size, self.weight_embedding_rank) / self.weight_embedding_rank))
    ...
    def forward(self, x, y_src):
        ...
        if self.low_rank_last:
            w2, next_res = take_weights(res, (second_shape, self.n_out))
        else:
